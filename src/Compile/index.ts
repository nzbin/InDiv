...
class Compile {
  ...
  public recursiveDOM(childNodes: NodeListOf<Node & ChildNode>, fragment: DocumentFragment | Element): void {
    Array.from(childNodes).forEach((node: Element) => {
      if (this.isElementNode(node) && this.$vm.$declarations.find((declaration: any) => declaration.$selector === node.tagName.toLocaleLowerCase() && declaration.$isComponentDirective)) node.isComponent = true;
